...
/** Paths in data.js are relative to the site root (index.html). */
...
function resolveHref(href) {
  ...
  if (href.startsWith("#")) {
    return isHomePage() ? href : `${resolvePath("index.html")}${href}`;
  }
...
function renderSiteHeader() {
  ...
  if (logoLink) {
    logoLink.href = isHomePage() ? "#topo" : resolvePath("index.html");
  }
  ...
  return `
      <section class="profile-section profile-video">
        ...
        <div class="profile-video__wrap">
          ...
          >
            <source src="${resolvePath(poet.video)}" type="video/mp4">
  ...
  return `
    <header class="profile-header">
      <div class="profile-photo">
        <img src="${resolvePath(poet.image)}" alt="Retrato de ${poet.name}" class="profile-photo__img">
...
function createPoetCardElement(poet) {
  ...
  imageWrap.appendChild(
    createPoetImageElement(resolvePath(poet.image), `Retrato de ${poet.name}`, initials)
  );
  ...
  link.href = resolvePath(`pages/poeta.html?id=${poet.id}`);
