/* ============================================================
   Yidish Velt Brasil · Página de Verbos
   Dados: regularVerbEndings, irregularVerbTables, verbsQuiz…
   ============================================================ */
...
function renderEndingsTable() {
  const labels = siteContent.verbsPage.endingsHeaders;
  const rows = regularVerbEndings
    .map(
      (row) => `
      <tr>
        <td><strong>${row.person}</strong></td>
        <td><code class="lesson-ending">${row.ending}</code></td>
        <td>${row.example}</td>
        <td>${row.rule}</td>
      </tr>`
    )
  ...
  document.getElementById("verbs-endings-wrap").innerHTML = `
    <table class="vocab-table">
      <thead>
        <tr>
          <th scope="col">${labels.person}</th>
          <th scope="col">${labels.ending}</th>
...
function renderExampleTable() {
  const headers = siteContent.pronounsPage.tableHeaders;
  const rows = regularVerbExample
    .map(
      (row) => `
      <tr>
        <td>${row.person}</td>
  ...
  document.getElementById("verbs-example-wrap").innerHTML = `
    <table class="vocab-table">
      <thead>
        <tr>
          <th scope="col">${headers.person}</th>
          <th scope="col">${headers.yiddish}</th>
          <th scope="col">${headers.translit}</th>
          <th scope="col">${headers.meaning}</th>
        </tr>
      </thead>
      <tbody>${rows}</tbody>
    </table>
...
function renderIrregularTables() {
  const headers = siteContent.pronounsPage.tableHeaders;
  const container = document.getElementById("verbs-irregular-tables");

  container.innerHTML = irregularVerbTables
    .map((table) => {
      ...
      return `
        <div class="lesson-subsection">
          <h3 class="lesson-section__subtitle">${table.title}</h3>
...
function renderOtherIrregularTable() {
  const labels = siteContent.verbsPage.otherHeaders;
  const rows = otherIrregularVerbs
    .map(
      (row) => `
      <tr>
        <td class="vocab-yiddish" lang="yi" dir="rtl">${row.yiddish}</td>
  ...
  document.getElementById("verbs-other-wrap").innerHTML = `
    <table class="vocab-table">
      <thead>
        <tr>
...
function renderVerbsPhrases() {
  document.getElementById("verbs-phrases").innerHTML = verbsPhrases
    .map(
      (phrase) => `
...
function getQuizLabels() {
  return siteContent.verbsPage.quiz;
}

function getCurrentQuestion() {
  return verbsQuiz[quizState.order[quizState.index]];
}
...
function formatProgress() {
  ...
  return labels.progressLabel
    ...
    .replace("{total}", String(verbsQuiz.length));
}

function buildQuizQuestionHTML() {
  const labels = getQuizLabels();
  const question = getCurrentQuestion();
  const progress = ((quizState.index + 1) / verbsQuiz.length) * 100;
...
function buildQuizResultHTML() {
  ...
  return `
    ...
    <p class="quiz-result-score">${quizState.score}/${verbsQuiz.length}</p>
...
function renderQuizStep() {
  ...
  if (quizState.index >= verbsQuiz.length) {
...
function wireQuizQuestion() {
  ...
  checkBtn.addEventListener("click", () => {
    ...
    checkBtn.textContent =
      quizState.index + 1 >= verbsQuiz.length ? labels.resultLabel : labels.nextLabel;
  });
}
...
function startQuiz() {
  quizState = {
    ...
    selected: null,
    checked: false,
    order: shuffleArray(verbsQuiz.map((_, i) => i)),
  };
  renderQuizStep();
}
...
function closeQuiz() {
  ...
  document.getElementById("verbs-practice-btn").focus();
}

function initQuizModal() {
  const modal = document.getElementById("quiz-modal");
  const closeBtn = document.getElementById("quiz-close");
  const practiceBtn = document.getElementById("verbs-practice-btn");
...
function initVerbsPage() {
  ...
  const content = siteContent.verbsPage;

  document.title = `${content.title} · ${siteContent.name}`;
  document.getElementById("verbs-back").textContent = content.backLabel;
  document.getElementById("verbs-title").textContent = content.title;
  document.getElementById("verbs-subtitle").textContent = content.subtitle;
  document.getElementById("verbs-regular-title").textContent = content.regularTitle;
  document.getElementById("verbs-regular-note").textContent = content.regularNote;
  document.getElementById("verbs-endings-title").textContent = content.endingsTitle;
  document.getElementById("verbs-example-title").textContent = content.exampleTitle;
  document.getElementById("verbs-irregular-title").textContent = content.irregularTitle;
  document.getElementById("verbs-irregular-note").textContent = content.irregularNote;
  document.getElementById("verbs-other-title").textContent = content.otherIrregularTitle;
  document.getElementById("verbs-phrases-title").textContent = content.phrasesTitle;
  document.getElementById("verbs-practice-btn").textContent = content.practiceBtn;

  renderEndingsTable();
  renderExampleTable();
  renderIrregularTables();
  renderOtherIrregularTable();
  renderVerbsPhrases();
  initQuizModal();
}

document.addEventListener("DOMContentLoaded", initVerbsPage);
